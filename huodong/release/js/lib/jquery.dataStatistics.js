$.fn.dataStatistics = function (options) {
	...
	ths.find('.dataStatistics_1 .digit_set').each(function () {
		for (i = 0; i <= 9; i++) {
			$(this).append(html);
			currentDigit = $(this).find('.digit')[i];
			$(currentDigit).find('.digit_wrap').append(i);
		...
		$.each(nowNums, function (index, val) {
		var set = ths.find('.dataStatistics_1 .digit_set').eq(index);
		var i = parseInt(val)
		set.find('.dataStatistics_1 .digit').eq(i).addClass('active');
		set.find('.dataStatistics_1 .digit').eq(i + 1).addClass('previous');
	});
