            ...
            if (_this.checkPhone($(".JS-phone"))) {
                ...
                $.ajax({
                    ...
                    success: function(d) {
                        ...
                        } else if(d.errcode == 512){
                        	_this.alertBox("您已注册，点击直接下载",3000);
        ...
        $(".search-btn").on("click", function() {
            if (_this.checkPhone($(".phone input")) && _this.checkCode($(".msg-code input"))) {
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    data: {
                        action: $("input.JS-code").length ? "register" : "lazyRegister",
                        phone: phoneNum,
                        code: codeNum,
                        place: getURLParams(window.location.href)["channel"]
                    },
                    success: function(d) {
                        if (d.errcode == 0) { //手机号码验证码都正确，跳转到H5查询
                        	$(".section").addClass("slideTop");
                        }else if (d.errcode == 512){
                        	_this.alertBox("您已注册，点击直接下载",3000);
