<?php 
	class Resource {
				...
				if ($this->mode === 'cdn') {
					if ($type === 'js') {
						foreach ($arr as $key => $value) {
							$value = 'https://' . $value;
							$resRes = $resRes . '<script src="' . $value . '"></script>' . "\n";
						}
						return $resRes;
					}else if ($type === 'css'){
						foreach ($arr as $key => $value) {
							$value = 'https://' . $value;
							$resRes = $resRes . '<link rel="stylesheet" href="' . $value . '">' . "\n";
						}
						return $resRes;
					}
		...
		// 打印数组中资源到html中 requirejs加载第三方库，活动项目单个js加载入口
		/*
		public function wrapHtmlTag($arr,$type){ 
			if (!$type) {
				return false;
			}
			if (isset($arr) && is_array($arr)) {
		...
		*/
		...
		function getResStr($assign){  // 拿指定活动的资源, common资源必拿
			if ($this->mode == 'local') {
				$res = dirname(__FILE__) . "/static/resource/resource.js";
			}else if($this->mode == 'cdn'){
				$res = dirname(__FILE__) . "/release/resource/resource.js";
			}else {
				return false;
			}
			...
			$assignResArr = $this->assingRes($resArr,$assign); // 指定资源，
